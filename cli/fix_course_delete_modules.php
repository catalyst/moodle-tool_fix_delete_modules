...
if (in_array('*', $taskids) || empty($taskids)) {
    ...
    $params = ['classname' => '\core_course\task\course_delete_modules'];
} else {
    [$sql, $params] = $DB->get_in_or_equal($taskids, SQL_PARAMS_NAMED, 'id');
    $params += ['classname' => '\core_course\task\course_delete_modules'];
...
$taskids = ($totaltaskscount == $taskcount) ? [] : $taskids;
